...
Future<GetPostShopItemDataResposne> HttpGetPostShopItemData(
    {@required
        GetPostShopItemDataRequest bufferGetPostShopItemDataRequest}) async {
  var body = bufferGetPostShopItemDataRequest.value();

  var url = Uri.parse("${HostName()}/billShopder/getPostShopItemsData");
  ...
  Map<String, PostShopItem_Inventory> bufferInventory =
      {}; //<inventory_id , data
  ...
  Map<String, PostShopItem_Address_user> bufferAddressUser = {}; //<address_us
  ...
  post_info = PostShopItem_PostShop(
  ...
  _inventory.forEach((key, value) {
    bufferInventory[key] = PostShopItem_Inventory(
  ...
  _menu.forEach((key, value) {
    bufferMenu[key] =
        PostShopItem_Menu(name: value['name'], path: value['path']);
  });
  ...
  _bill.forEach((key, value) {
    ...
    if (value['date_confirm'] == null) {
      bufferBill[key] = PostShopItem_Bill(
  ...
  _item.forEach((key, value) {
    bufferItem[key] = PostShopItem_Item(
  ...
  _payment.forEach((key, value) {
    DateBox date_payment = DateBox(
        ...
        min: value['date']['min'],
        sec: value['date']['sec']);
    bufferPayment[key] = PostShopItem_Payment(
        bill_id: value['bill_id'],
        ref: value['ref'],
        cost: value['cost'],
        date: date_payment,
  ...
  _users.forEach((key, value) {
    bufferUsers[key] =
        PostShopItem_Users(name: value['name'], path: value['path']);
  });

  _address_user.forEach((key, value) {
    bufferAddressUser[key] = PostShopItem_Address_user(
        user_id: value['user_id'],
        name: value['name'],
        phone: value['phone'],
  ...
  return GetPostShopItemDataResposne(
      post_info: post_info,
