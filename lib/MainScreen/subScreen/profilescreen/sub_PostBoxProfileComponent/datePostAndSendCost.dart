...
class _DatePostAndSendCostState extends State<DatePostAndSendCost> {
  ...
  Widget build(BuildContext context) {
    ...
    Widget DateSendComponent = Container(
      ...
      child:
          Text("วันที่จัดส่ง${this.widget.postShopData_post.send.ToString()}"),
    );
    return Container(
              ...
              children: [
                Container(
                    margin: EdgeInsets.only(
                        left: 5, top: 10, bottom: 10), //ส่งถึงที่/รับที่ร้าน
                    child: Text(
                        "${how_send_item[this.widget.postShopData_post.how_send]}")),
              ],
            )
          ],
        ));
